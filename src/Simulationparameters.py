...
class SimulationParameters:
    ...
    def load_file(self, filename: str):
        ...
        self.warehouses = self.create_warehouse_list(lines[basic_info_lines_amount:])
        self.drones = self.create_drone_list(no_drones, self.warehouses[0], self.max_payload)
        self.orders = self.create_order_list(lines[basic_info_lines_amount + 2 * len(self.warehouses) + 1:])

    def create_warehouse_list(self, lines: [str]) -> [Warehouse]:
    ...
    def create_drone_list(self, no_drones: int, home_warehouse: Warehouse, max_payload: int) -> [Drone]:
        drones = []
        for i in range(0, no_drones):
            drones.append(Drone(i, home_warehouse.coords, max_payload))
        assert len(drones) == no_drones
        return drones

    def create_order_list(self, lines) -> [Order]:
